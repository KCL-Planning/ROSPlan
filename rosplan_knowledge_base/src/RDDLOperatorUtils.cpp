...
namespace KCL_rosplan {
    ...
    PosNegDomainFormula RDDLOperatorUtils::getOperatorPreconditions(const std::string &op_name,
                                                                    const std::vector<LogicalExpression *> &SACs) {
        PosNegDomainFormula ret;

        for (auto it = SACs.begin(); it != SACs.end(); ++it) {
            auto precs = getOperatorPrecondition(op_name, *it);
            join(ret, precs);
        }
    ...
    PosNegDomainFormula RDDLOperatorUtils::getOperatorPrecondition(const std::string &op_name, const LogicalExpression *SAC) {
        auto exp_con = dynamic_cast<const Connective *>(SAC);
        if (exp_con != nullptr) return getOperatorPrecondition(op_name, exp_con);

        auto exp_neg = dynamic_cast<const Negation *>(SAC);
        if (exp_neg != nullptr) return getOperatorPrecondition(op_name, exp_neg);

        auto exp_quan = dynamic_cast<const Quantifier *>(SAC);
        if (exp_quan != nullptr) return getOperatorPrecondition(op_name, exp_quan->expr);

        /*auto exp_param = dynamic_cast<const ParametrizedVariable *>(SAC);
        if (exp_param != nullptr) return getOperatorPrecondition(op_name, exp_param);

        auto exp_numeric = dynamic_cast<const NumericConstant *>(SAC);
        if (exp_numeric != nullptr) return getOperatorPrecondition(op_name, exp_numeric);*/

        return PosNegDomainFormula();
    }


    PosNegDomainFormula RDDLOperatorUtils::toDomainFormula(const LogicalExpression *expr) {
        auto exp_con = dynamic_cast<const Conjunction *>(expr);
        if (exp_con != nullptr) {
            assert(exp_con->exprs.size() == 2);
            PosNegDomainFormula op1, op2;
            op1 = toDomainFormula(exp_con->exprs[0]);
            op2 = toDomainFormula(exp_con->exprs[1]);
            join(op1, op2);
            return op1;
        }
        ...
        if (exp_neg != nullptr) {
            PosNegDomainFormula neg = toDomainFormula(exp_neg->expr);
            negate(neg);
            return neg;
        }
    ...
    PosNegDomainFormula RDDLOperatorUtils::getOperatorPrecondition(const std::string &op_name, const Connective *SAC) {
        ...
        if (disj != nullptr) {
            ...
            if (neg != nullptr) {
                ...
                if (action != nullptr and action->variableType == ParametrizedVariable::ACTION_FLUENT and
                    action->variableName == op_name) {
                    return toDomainFormula(disj->exprs[1]);
                }
            }
        }

        PosNegDomainFormula op1 = getOperatorPrecondition(op_name, SAC->exprs[0]);
        PosNegDomainFormula op2 = getOperatorPrecondition(op_name, SAC->exprs[1]);
        join(op1, op2);
        return op1;
    }


    PosNegDomainFormula RDDLOperatorUtils::getOperatorPrecondition(const std::string &op_name, const Negation *SAC) {
        // Swap the resulting precondition
        PosNegDomainFormula neg = getOperatorPrecondition(op_name, SAC->expr);
        negate(neg);
        return neg;
    }

    PosNegDomainFormula RDDLOperatorUtils::getOperatorEffects(const std::string &op_name,
        ...
        for (auto it = CPFs.begin(); it != CPFs.end(); ++it) {
            PosNegDomainFormula eff_i = getOperatorEffects(op_name, it->first, it->second);
            join(eff_ret, eff_i);
        }
        return eff_ret;
    }
    ...
    PosNegDomainFormula
    RDDLOperatorUtils::getOperatorEffects(const std::string& op_name, const ParametrizedVariable *pVariable, const LogicalExpression *exp) {
        auto ifte = dynamic_cast<const IfThenElseExpression*>(exp);
        if (ifte != nullptr) return getOperatorEffects(op_name, pVariable, ifte);

        auto con = dynamic_cast<const Connective*>(exp);
        if (con != nullptr) { // FIXME sure?
            assert(con->exprs.size() == 2);
            PosNegDomainFormula a, b;
            a = getOperatorEffects(op_name, pVariable, con->exprs[0]);
            b = getOperatorEffects(op_name, pVariable, con->exprs[1]);
            join(a, b);
            return a;
        }

        auto exist = dynamic_cast<const ExistentialQuantification*>(exp);
        if (exist != nullptr) return getOperatorEffects(op_name, pVariable, exist->expr);

        auto neg = dynamic_cast<const Negation*>(exp);
        if (neg != nullptr) { // Swap add for del
            PosNegDomainFormula neg_expr = getOperatorEffects(op_name, pVariable, neg->expr);
            negate(neg_expr);
            return neg_expr;
        }
        ...
        if (var != nullptr) {
            if ((var->variableType == ParametrizedVariable::ACTION_FLUENT) and (var->variableName == op_name))
                return toDomainFormula(pVariable);
    ...
    PosNegDomainFormula
    RDDLOperatorUtils::getOperatorEffects(const std::string& op_name, const ParametrizedVariable *pVariable, const IfThenElseExpression *exp) {
        ...
        PosNegDomainFormula effects = getOperatorEffects(op_name, pVariable, exp->condition); // Checks if the condition has some implications on the effects of the operator
        ...
        if (iftrue != nullptr) {
            ...
            PosNegDomainFormula res_if = getOperatorEffects(op_name, pVariable, iftrue);
            join(ret, res_if);
        }
        ...
        if (iffalse != nullptr) { // We have a true/False value
            ...
            PosNegDomainFormula elseif_result = getOperatorEffects(op_name, pVariable, elseif);
            join(ret, elseif_result);
        }
        else {
            PosNegDomainFormula res_else = getOperatorEffects(op_name, pVariable, iftrue);
            join(ret, res_else);
        }

        return ret;
    }
// FIXME add op_name to effects!?
