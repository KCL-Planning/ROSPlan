#include "rosplan_planning_system/POPF3EsterelPlanParser.h"

/* implementation of rosplan_planning_system::POPF3EsterelPlanParser.h */
namespace KCL_rosplan {

	/* constructor */
	POPF3EsterelPlanParser::POPF3EsterelPlanParser(ros::NodeHandle &nh) : node_handle(&nh)
	{
		// knowledge interface
		update_knowledge_client = nh.serviceClient<rosplan_knowledge_msgs::KnowledgeUpdateService>("/kcl_rosplan/update_knowledge_base");
	}

	void POPF3EsterelPlanParser::reset() {
		plan_nodes.clear();
		plan_edges.clear();
	}

	void POPF3EsterelPlanParser::generateFilter(PlanningEnvironment &environment) {
		// do nothing yet
	}

	void POPF3EsterelPlanParser::toLowerCase(std::string &str) {
	 ...
	 */
	void POPF3EsterelPlanParser::preparePDDLConditions(StrlNode &node, PlanningEnvironment &environment) {
	...
	void POPF3EsterelPlanParser::createNodeAndEdge(const std::string& action_name, double dispatchTime, double duration, int node_id, PlanningEnvironment &environment, StrlNode& node, StrlEdge& edge) {
	 ...
	 */
	void POPF3EsterelPlanParser::preparePlan(std::string &dataPath, PlanningEnvironment &environment, size_t freeActionID) {

		ROS_INFO("KCL: (POPF3EsterelPlanParser) Loading plan from file: %s. Initial action ID: %zu", ((dataPath + "plan.pddl").c_str()), freeActionID);
		...
		while(!infile.eof()) {
			...
			if (line.substr(0,6).compare("; time") == 0) {
		...
		{
			ROS_WARN("KCL: (POPF3EsterelPlanParser) No suitable plan structure was found.");
		}
	 ...
	 */
	bool POPF3EsterelPlanParser::produceEsterel() {
		...
		ROS_INFO("KCL: (POPF3EsterelPlanParser) Write the esterel plan: %s", strl_file.c_str());
