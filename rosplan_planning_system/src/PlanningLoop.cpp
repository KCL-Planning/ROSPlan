...
namespace KCL_rosplan {
	...
	bool runPlanner(std::string &dataPath, std::string &domainPath, std::string &problemPath)
	{
		...
		if(KCL_rosplan::actionList.size() > 0) {
			std::vector<rosplan_dispatch_msgs::ActionDispatch> oldplan(KCL_rosplan::actionList);
	...
	void runPlanningServer()
	{
		...
		actionPublisher = nh.advertise<rosplan_dispatch_msgs::ActionDispatch>("/kcl_rosplan/action_dispatch", 1000, true);
		...
		filterPublisher = nh.advertise<rosplan_knowledge_msgs::Filter>("/kcl_rosplan/filter", 10, true);
		...
		while (ros::ok() && KCL_rosplan::actionList.size() > KCL_rosplan::currentAction) {
			...
			rosplan_dispatch_msgs::ActionDispatch currentMessage = KCL_rosplan::actionList[KCL_rosplan::currentAction];
