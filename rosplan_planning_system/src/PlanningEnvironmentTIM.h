...
namespace KCL_rosplan
{
	...
	void parseDomain(const std::string domainPath) {
		...
		// parse domain
		// TODO this fails as it expects there to also be a problem file
		TIM::performTIMAnalysis(&current_filename);
		for_each(TIM::TA->pbegin(),TIM::TA->pend(), ptrwriter<TIM::PropertySpace>(cout,"\n"));
		for_each(TIM::TA->abegin(),TIM::TA->aend(), ptrwriter<TIM::PropertySpace>(cout,"\n"));	
